...
@export var color: Color = Color(1,1,1,1) # The color of the sunlight
@export var gradient: Gradient # The color gradient to use

@export var render_3D: bool = true # Select wich sun object to render
@export var color_gradient: bool = true # Change color according to rotation
...
func _ready() -> void:
	...
	# Setup the sun location and relative size
...
func _physics_process(delta: float) -> void:
	# Rotate the directional light, moving at the same time the sun rendered
	var rotation_delta = rotation_degree * delta
	rotate_x(deg_to_rad(rotation_delta))
	
	# Compute the new color if needed
	if color_gradient:
